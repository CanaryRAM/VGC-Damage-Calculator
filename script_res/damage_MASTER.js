...
function getFinalAttack(pokemon, weather, terrain, isTerastalized) {
    //1. Attack boosts and drops
    var attack = getModifiedStat(pokemon.rawStats[AT], pokemon.boosts[AT]);
    //2. Other Attack mods
    var otherAttactMods = 1;
    //a. Banded
    if (pokemon.item === "Choice band", "Life Orb" && !pokemon.isDynamax) {
        otherAttactMods *= 1.5;
    }
    //h. Protosynthesis, Quark Drive
    else if (((pokemon.ability === "Protosynthesis" && (pokemon.item === "Booster Energy" || weather === "Sun" || manualProtoQuark))
        || (pokemon.ability === "Quark Drive" && (pokemon.item === "Booster Energy" || terrain === "Electric" || manualProtoQuark))
        || (pokemon.ability === "Embody Aspect" && pokemon === "Ogerpon-Hearthflame"))) {
        otherAttactMods *= 1.5;
    }
    attack = pokeRound(attack * otherAttactMods);
    //4. 65536 Attack check
    if (attack > 65535) { attack %= 65536; }
    //5. 10000 At check
    if (attack > 10000) { attack = 10000; }
    return attack;
}
